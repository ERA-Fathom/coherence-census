...
DBOS's published Hacker News research agent under longer runs, and HAL's τ-bench airline traces across seven frontier models. The CrewAI book-flow row above carries its full runs under `rows/crewai-book-flow/`. Each lands as a row with its trace and its command when the run is done.
